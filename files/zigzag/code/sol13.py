...
class Breakthrough():
      ...
      try:
        with open(saveName,"w") as saveFH:
          ...
          if self.__Discard.GetNumberOfCards() > 0:
            discard = ",".join([f"{self.__Discard.GetCardDescriptionAt(i)} {self.__Discard.GetCardNumberAt(i)}" for i in range(self.__Discard.GetNumberOfCards())])
          else:
            discard = ""
    ...
    def __GetChoice(self):
        print()
        # CHANGE
        Choice = input("(D)iscard inspect, (S)ave or (U)se card?:> ").upper()
        # END CHANGE
        return Choice
...
class Lock():
    ...
    def GetChallengesAsString(self):
      ...
      for C in self._Challenges:
        if len(challenges) > 0:
          challenges += ";"
        challenges += self.__ConvertConditionToString(C.GetCondition())
      return challenges
