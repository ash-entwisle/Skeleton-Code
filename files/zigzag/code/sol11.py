        ...
        if self.__Deck.GetNumberOfCards() > 0:
            ...
            if self.__Credits >= 2:
                ...
                if Choice.strip().lower() == "y":
                    ToolList = self.__Deck.PrintToolsAvailable(True if self.__Credits >= 3 else False)
                    CardChosen = int(input("Which tool would you like to buy? "))
                    if CardChosen != 10 and ToolList[CardChosen -1] != -1:
                        self.__MoveCard(self.__Deck, self.__Hand, self.__Deck.GetCardNumberAt(ToolList[CardChosen-1]))
                        if CardChosen > 6:
                            self.__Credits -= 3
                        else:
                            self.__Credits -= 2
    ...
    def PrintToolsAvailable(self, KeysAvailable):
        ...
        for i in range(self.GetNumberOfCards()):
            if self._Cards[i].GetDescription() == "F a":
                ...
                if ToolList[5] == -1:
                    ToolList[5] = i
            elif self._Cards[i].GetDescription() == "K a" and KeysAvailable:
                ToolsAvailable[6] += 1
                if ToolList[6] == -1:
                    ToolList[6] = i
            elif self._Cards[i].GetDescription() == "K b" and KeysAvailable:
                ToolsAvailable[7] += 1
                if ToolList[7] == -1:
                    ToolList[7] = i
            elif self._Cards[i].GetDescription() == "K c" and KeysAvailable:
                ToolsAvailable[8] += 1
                if ToolList[8] == -1:
                    ToolList[8] = i
        ...
        for i in range(len(Tools)):
            if ToolList[i] >= 0:
                print(f"{i+1}. {Tools[i]} ({ToolsAvailable[i]} available)")
