    ...
    def __GetCardFromDeck(self, CardChoice):
        if self.__Deck.GetNumberOfCards() > 0:
            ...
            # CODE ADDED
                ...
                print("You can either use this card immediately to solve a challenge or discard it.")
                Choice = input(f"enter 1-{self.__CurrentLock.GetNumberOfChallenges()} to solve a challenge or (D)iscard it so it can come up after reshuffling:> ").upper()
                if Choice.strip() == "D":
                    self.__Discard.AddCard(CurrentCard)
                else:
                    CurrentCard.Process(self.__CurrentLock, int(Choice))
            # END ADDITION
